...
const FEED_EVENTS = [
  {
    tag: "DEEPFAKE",
    color: "red",
    msg: "facial artifact detected · confidence 94.2%",
  },
  {
    tag: "VERIFIED",
    color: "green",
    msg: "claim cross-referenced · 12 sources match",
  },
  {
    tag: "DMCA",
    color: "amber",
    msg: "stolen content · 3 upstream sources found",
  },
  { tag: "GAN", color: "red", msg: "GAN signature present · midjourney v6" },
  {
    tag: "METADATA",
    color: "amber",
    msg: "EXIF timestamp mismatch · 47 days delta",
  },
  {
    tag: "VERIFIED",
    color: "green",
    msg: "video authentic · 2,048 frames clean",
  },
  {
    tag: "DEEPFAKE",
    color: "red",
    msg: "lip-sync anomaly detected · frame 0312",
  },
  {
    tag: "CLAIM",
    color: "amber",
    msg: "partially false · 2 of 5 claims unverified",
  },
  { tag: "PHASH", color: "green", msg: "no duplicates found · unique content" },
  {
    tag: "DEEPFAKE",
    color: "red",
    msg: "lighting inconsistency · right facial plane",
  },
  {
    tag: "VERIFIED",
    color: "green",
    msg: "journalist source verified · reuters",
  },
  {
    tag: "ALERT",
    color: "red",
    msg: "high-severity · escalated to review queue",
  },
];
...
function LiveFeed() {
  const [items, setItems] = useState<
    ((typeof FEED_EVENTS)[0] & { id: number })[]
  >([]);
  const counterRef = useRef(0);
  const idRef = useRef(0);

  useEffect(() => {
    const seed = FEED_EVENTS.slice(0, 4).map((e) => ({
      ...e,
      id: idRef.current++,
    }));
    setItems(seed);
    ...
    const iv = setInterval(() => {
      const next = {
        ...FEED_EVENTS[counterRef.current % FEED_EVENTS.length],
        id: idRef.current++,
      };
      counterRef.current++;
      ...
      <AnimatePresence initial={false}>
        {items.map((item) => (
          ...
          >
            ...
            <span className={`tag tag-${item.color} lp-feed__tag`}>
              {item.tag}
            </span>
...
function Field({
  ...
  return (
    <motion.div className="lp-field" variants={FieldVariants}>
      <label htmlFor={id} className="mono lp-label">
        {label}
      </label>
      <input
        id={id}
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Client-side password validation for register (mirrors server-side rules)
    if (mode === "register") {
        ...
        <div className="lp-left__top">
          ...
          >
            ...
            <p className="lp-subline">
              Live detections from the global scan queue.
            </p>
          </motion.div>
        </div>
